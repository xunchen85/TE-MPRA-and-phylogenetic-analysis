        ...
        for i, read in tqdm(enumerate(bam), 'paired-end reads', total = n_bam_records):
            ...
            if read.mapping_quality > int(mapq):
                if cigar == "":
                    ...
                    # Sum up all aligned bases from CIGAR string
                    aligned_length = sum(length for op, length in read.cigartuples if op in [0, 7, 8])  # M, =, X operations
                    if aligned_length == read.query_length:  # Check if all bases are aligned
                        if isinstance(read.reference_name, str):
                            query_to_coords[read.query_name] = read.reference_name           
